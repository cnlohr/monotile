...
TARGS := app/authback.cgi app/logout.cgi app/test.cgi app/test_federate.cgi app/plot.cgi
all : $(TARGS)
...
install : $(TARGS)
	...
	echo "CREATE TABLE IF NOT EXISTS logins( lid INTEGER PRIMARY KEY AUTOINCREMENT, key TEXT, name TEXT NOT NULL, userurl TEXT, logintime DATE NOT NULL )" | sqlite3 data/database/monotile.db
	...
	echo "CREATE TABLE IF NOT EXISTS pixels( pid INTEGER PRIMARY KEY AUTOINCREMENT, lid INTEGER NOT NULL, cx INTEGER NOT NULL, cy INTEGER NOT NULL, bx INTEGER NOT NULL, by INTEGER NOT NULL, dtime DATE NOT NULL, removed BOOLEAN )" | sqlite3 data/database/monotile.db
